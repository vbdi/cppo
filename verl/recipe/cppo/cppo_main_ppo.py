...
from recipe.cppo.reward import load_reward_manager
from verl.trainer.ppo.utils import need_critic, need_reference_policy
from verl.utils.config import validate_config
from verl.utils.device import auto_set_ascend_device_name, is_cuda_available
from verl.utils.import_utils import load_extern_object

from recipe.cppo.cppo_ray_trainer import RayCPPOTrainer

@hydra.main(config_path="config", config_name="cppo_trainer", version_base=None)
def main(config):
...
def run_ppo(config, task_runner_class=None) -> None:
    ...
    if task_runner_class is None:
        task_runner_class = ray.remote(num_cpus=1)(CPPO_TaskRunner)  # please make sure main_task is not scheduled on head
...
class CPPO_TaskRunner:
    ...
    def add_actor_rollout_worker(self, config):
        ...
        if config.actor_rollout_ref.actor.strategy in {"fsdp", "fsdp2"}:
            from recipe.cppo.cppo_fsdp_workers import ActorRolloutRefWorker, AsyncActorRolloutRefWorker
