# Adapted from torchtitan: torchtitan/models/moe/kernels.py
...
def fill_indices_wrapper(
    ...
    """Wrapper for Triton kernel to fill permutation indices.

    Args:
        tokens_per_expert_group: Number of tokens for each expert from all ranks.
        start_index_values: Start index for each expert-rank pair.
        write_offsets: Write offset for each local expert.
        experts_per_rank: Number of experts per rank.
        num_ranks: Number of ranks.
        max_len: Maximum length of the output index vector.
        block_size: Block size for Triton kernel.
        max_blocks: Maximum number of blocks to launch.

    Returns:
        Permutation indices tensor.
    """
    # preallocate output
...
def fill_indices_cpu(
    ...
    """CPU reference implementation for fill_indices.
    ...
    Returns:
        Permutation indices tensor on CPU.
    """
    permuted_indices = torch.full(
        (max_len,),
        -1,
...
def generate_permute_indices(
    ...
    Args:
        ...
        num_ranks: number of ranks.
        max_len: maximum length of the output index vector.
        alignment: alignment for each returned element in `m_sizes` and padding
            min for zero token experts.
        use_cpu: whether to use CPU implementation.

    Returns:
        permuted_indices: Tensor of indices that map original token order to the
            expert-grouped order.
        m_sizes: aligned number of tokens for each expert (padded to alignment
            boundary).
        m_offsets: Cumulative sum of m_sizes. The exclusive ending position for
            each expert's tokens.

    Explanatory details:
        `tokens_per_expert_group` is of shape (num_ranks * experts_per_rank,),
        for example:
        From: |       rank 0      |       rank 1      |
