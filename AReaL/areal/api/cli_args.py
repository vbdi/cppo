...
class PPOActorConfig(TrainEngineConfig):
    ...
    # CPPO (Contrastive Preference Pair Optimization) hyperparameters
    cpl_loss_coef: float = field(
        default=0.01,
        metadata={"help": "Coefficient for CPL"},
    )
    cpl_use_vision_mask: bool = field(
        default=True,
        metadata={"help": "Whether to apply vision masking in contrastive loss computation"},
    )
    cpl_vision_top_percent: float = field(
        default=0.5,
        metadata={"help": "Percentage of top positive values to keep in vision masking"},
    )
    cpl_use_advantage_gating: bool = field(
        default=True,
        metadata={"help": "Whether to gate vision loss by positive advantages"},
    )
    cpl_tau: float = field(
        default=0.1,
        metadata={"help": "Temperature parameter for contrastive loss softplus"},
    )
