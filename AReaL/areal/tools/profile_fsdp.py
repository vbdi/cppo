...
in HuggingFace model forward and backward passes using torch.profiler.

This tool loads models the same way as FSDP engine (via AutoModelForCausalLM)
for fair comparison with Archon engine profiling.

Usage:
    python -m areal.tools.profile_fsdp [OPTIONS]
...
Examples:
    ...
    # Forward only
    python -m areal.tools.profile_fsdp --mode forward

    # Variable-length sequences
    python -m areal.tools.profile_fsdp --seq-lens 128,256,512,128

    # Custom output path
    python -m areal.tools.profile_fsdp --output /tmp/my_trace.json

    # Long sequence test
    python -m areal.tools.profile_fsdp --batch-size 1 --total-len 16384
"""
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Profile FSDP/HuggingFace model forward/backward to identify performance bottlenecks.",
...
""",
    ...
    parser.add_argument(
        ...
        "Defaults to ./profile_fsdp_<timestamp>.json",
    )
    parser.add_argument(
        "--no-memory",
        action="store_true",
...
def get_output_path(output: str | None, mode: str) -> Path:
    ...
    return Path(f"./profile_fsdp_{mode}_{timestamp}.json")
...
) -> tuple[torch.nn.Module, AutoConfig]:
    """Load HuggingFace model (same as FSDP engine).

    Args:
        model_path: Path or repo ID of the model.
...
def create_padded_input(
    ...
    """Create padded batch input (HuggingFace style).
    ...
    Returns:
        dict with input_ids, attention_mask, and seq_lens.
    """
    if seq_lens is None:
    ...
    # Create padded batch
    input_ids = torch.zeros((batch_size, max_seqlen), dtype=torch.long, device=device)
    attention_mask = torch.zeros(
        (batch_size, max_seqlen), dtype=torch.long, device=device
    )

    for i, seq_len in enumerate(seq_lens):
        input_ids[i, :seq_len] = torch.randint(
            100, vocab_size - 100, (seq_len,), device=device
        )
...
def run_profile(args: argparse.Namespace) -> None:
    ...
    # Get model path - use same default as Archon for fair comparison
    ...
    print("\n" + "=" * 70)
    print(f"FSDP/HF PROFILING ({mode_str})")
    print("=" * 70)
    ...
    # Load model
    print("\n[Profile] Loading HuggingFace model...")
    model, config = load_hf_model(
        model_path,
        dtype=dtype,
    ...
    print(f"  Total tokens (non-padded): {sum(inputs['seq_lens'])}")
    print(f"  Max sequence length: {inputs['max_seqlen']}")
    print(f"  Padded shape: {list(inputs['input_ids'].shape)}")
